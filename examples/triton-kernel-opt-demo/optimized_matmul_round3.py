...
VARIANT_NAME = "round3"
MAIN_CHANGE = "Round 3 keeps grouped ordering and adds a full-tile fast path that removes boundary masks on aligned shapes."
AUTOTUNE_FULL_TILE_BLOCK_M = 256
AUTOTUNE_FULL_TILE_BLOCK_N = 256
AUTOTUNE_FULL_TILE_BLOCK_K = 64
...
def get_best_config(a: torch.Tensor, b: torch.Tensor) -> dict:
    ...
    matmul(a, b)
    return _serialize_config(round3_matmul_kernel.best_config)


def _matmul_fixed_config(a: torch.Tensor, b: torch.Tensor, fixed_config: dict) -> torch.Tensor:
    M, N, K, c = _make_output(a, b)
    grid = _make_grid(M, N)
    block_m = fixed_config["kwargs"]["BLOCK_M"]
    block_n = fixed_config["kwargs"]["BLOCK_N"]
    block_k = fixed_config["kwargs"]["BLOCK_K"]
    round3_matmul_kernel.fn[grid](
        a,
        b,
        c,
        M,
        ...
        c.stride(0),
        c.stride(1),
        FULL_TILES=(M % block_m == 0 and N % block_n == 0 and K % block_k == 0),
        **fixed_config["kwargs"],
...
def matmul(a: torch.Tensor, b: torch.Tensor, fixed_config: dict | None = None) -> torch.Tensor:
    ...
    round3_matmul_kernel[grid](
        ...
        c.stride(0),
        c.stride(1),
        # The autotuned launch uses a conservative gate that is only true
        # when the input shape is aligned for every candidate tile size.
        FULL_TILES=(
            M % AUTOTUNE_FULL_TILE_BLOCK_M == 0
            and N % AUTOTUNE_FULL_TILE_BLOCK_N == 0
            and K % AUTOTUNE_FULL_TILE_BLOCK_K == 0
        ),
    )
    return c
