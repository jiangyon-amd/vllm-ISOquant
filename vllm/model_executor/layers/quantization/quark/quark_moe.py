...
class QuarkOCP_MX_MoEMethod(QuarkMoEMethod):
    ...
    def apply(
        ...
        if not self.emulate:
            ...
            # Unified path: rotation is passed through aiter fused_moe
            # which handles rotation+quant+sort internally via fused kernel.
            # No rotation → rotation=None, aiter uses original quant path.
            out = rocm_aiter_fused_experts(
                ...
                expert_map=layer.expert_map,
                rotation=rotation,
                rotation_size=rotation_size if rotation is not None else 0,
            )
        else:
            from vllm.model_executor.layers.fused_moe import fused_experts
