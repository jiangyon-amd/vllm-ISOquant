#!/usr/bin/env python3
"""Test different NUM_KV_SPLITS values."""
...
def main():
    ...
    print(f"\n=== B={B}, seq_len={seq_len} ===")
    print(f"Testing different NUM_KV_SPLITS values:")
    
    for NUM_KV_SPLITS in [1, 2, 4, 8, 16, 32]:
        mid_o = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
        run_fn = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v12.so"), 
                                q_rot, kv_cache, bt, sls, centroids_f32, mid_o, 
                                B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
        run_fn(); torch.cuda.synchronize()
        us = bench(f"NUM_KV_SPLITS={NUM_KV_SPLITS}", run_fn)
