#!/usr/bin/env python3
"""Benchmark: Fused V4 HIP kernel vs Triton."""
...
def run_fused_v4_kernel(so_path, q_rot, kv_cache, bt, sls, centroids_f32, mid_o, output, sync_counter, B, Hq_l, NUM_KV_SPLITS):
    lib = ctypes.CDLL(so_path)
    launch_fn = lib.launch_tq_decode_fused_v4
    launch_fn.argtypes = (
...
def main():
    ...
    so_path = os.path.join(HIP_DIR, "tq_decode_fused_v4.so")
    try:
        run_fn = run_fused_v4_kernel(so_path, q_rot, kv_cache, bt, sls, centroids_f32, mid_o, output, sync_counter, B, Hq, NUM_KV_SPLITS)
        run_fn()
        torch.cuda.synchronize()
        ...
        fused_us = bench("HIP Fused V4", lambda: (run_fn(), torch.cuda.synchronize()))
        print(f"  Speedup vs Triton: {triton_us/fused_us:.2f}x")
        print(f"  Savings: {triton_us - fused_us:.1f} us")
        
    except Exception as e:
        print(f"  HIP Fused V4: FAILED ({e})")
        import traceback
        traceback.print_exc()
