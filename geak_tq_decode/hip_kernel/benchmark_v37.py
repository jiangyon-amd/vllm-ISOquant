...
def main():
    ...
    v36_us = bench("HIP v36 (2 warps)", run_v36)

    # v37 (3 warps)
    mid_o_v37 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v37 = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v37.so"), q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v37, B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
    run_v37(); torch.cuda.synchronize()
    v37_us = bench("HIP v37 (3 warps)", run_v37)
