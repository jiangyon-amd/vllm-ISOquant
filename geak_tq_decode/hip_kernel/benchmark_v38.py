...
def main():
    ...
    v36_us = bench("HIP v36 (2 warps, shared)", run_v36)

    # v38 (2 warps, shuffle)
    mid_o_v38 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v38 = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v38.so"), q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v38, B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
    run_v38(); torch.cuda.synchronize()
    v38_us = bench("HIP v38 (2 warps, shuffle)", run_v38)
    
    # Check correctness
    diff = (mid_o_v36 - mid_o_v38).abs().max().item()
    print(f"\n  Max diff v36 vs v38: {diff:.6f}")
