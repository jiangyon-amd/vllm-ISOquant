...
def main():
    ...
    run_v36(); torch.cuda.synchronize()
    v36_us = bench("HIP v36 (split range)", run_v36)

    # v39 (2 warps, interleaved)
    mid_o_v39 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v39 = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v39.so"), q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v39, B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
    run_v39(); torch.cuda.synchronize()
    v39_us = bench("HIP v39 (interleaved)", run_v39)
    
    # Check correctness
    diff = (mid_o_v36 - mid_o_v39).abs().max().item()
    print(f"\n  Max diff v36 vs v39: {diff:.6f}")
