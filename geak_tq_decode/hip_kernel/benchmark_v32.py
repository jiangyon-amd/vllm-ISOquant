...
def main():
    ...
    v12_us = bench("HIP v12 (baseline, 32 threads)", run_v12)

    # v32
    mid_o_v32 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v32 = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v32.so"), q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v32, B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
    run_v32(); torch.cuda.synchronize()
    v32_us = bench("HIP v32 (64 threads)", run_v32)
    
    print(f"\n  Speedup v32 vs v12: {v12_us/v32_us:.2f}x")
