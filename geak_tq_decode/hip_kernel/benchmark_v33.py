...
def main():
    ...
    v12_us = bench("HIP v12 (baseline, BLOCK_KV=8)", run_v12)

    # v33
    mid_o_v33 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v33 = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v33.so"), q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v33, B, Hq, NUM_KV_SPLITS, BLOCK_KV=4)
    run_v33(); torch.cuda.synchronize()
    v33_us = bench("HIP v33 (BLOCK_KV=4)", run_v33)
    
    print(f"\n  Speedup v33 vs v12: {v12_us/v33_us:.2f}x")
