#!/usr/bin/env python3
"""Tune v34 with different NUM_KV_SPLITS values."""
...
def run_hip_kernel(so_path, q_rot, kv_cache, bt, sls, centroids_f32, mid_o,
                   B, Hq_l, NUM_KV_SPLITS, BLOCK_KV=4):
...
def main():
    ...
    print(f"\n=== B={B}, seq_len={seq_len} ===")
    print(f"Testing v34 with different NUM_KV_SPLITS:")
    
    for NUM_KV_SPLITS in [4, 8, 16, 32]:
        mid_o = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
        run_fn = run_hip_kernel(os.path.join(HIP_DIR, "tq_decode_stage1_v34.so"), 
                                q_rot, kv_cache, bt, sls, centroids_f32, mid_o, 
                                B, Hq, NUM_KV_SPLITS, BLOCK_KV=4)
        run_fn(); torch.cuda.synchronize()
        us = bench(f"v34 NUM_KV_SPLITS={NUM_KV_SPLITS}", run_fn)
