#!/usr/bin/env python3
"""Benchmark v56 (GEMV-fused Stage1) + HIP Stage2(bf16) vs current full pipeline"""
...
def main():
    ...
    fn_v56.argtypes = (
        ...
        + [ctypes.c_int] * 3   # norm_correction, B, Hq
        + [ctypes.c_void_p]    # stream
    )
    fn_v56.restype = None

    # Load HIP Stage2 bf16
    ...
    print(f"\n{'='*110}")
    print(f"  {'Config':<16s} | {'OLD (GEMM+S1+TS2+bf16)':>22s} | {'NEW (v56+HS2bf16)':>18s} | {'Saved':>8s} | {'SDPA':>8s} | {'OLD/SDPA':>9s} | {'NEW/SDPA':>9s}")
    print(f"{'='*110}")

    for B, seq_len in [(1, 512), (1, 1024), (1, 2048), (1, 4096), (4, 4096), (8, 512), (32, 512), (64, 512)]:
        ...
        output_bf16_old = torch.empty(B, Hq, D, dtype=torch.bfloat16, device=DEVICE)
        output_bf16_new = torch.empty(B, Hq, D, dtype=torch.bfloat16, device=DEVICE)
        ...
        # OLD: q.float() + GEMM + HIP Stage1 + Triton Stage2 + bf16 cast
        def run_old():
            ...
            output_bf16_old.copy_(output_f32)

        # NEW: v56 (GEMV fused) + HIP Stage2(bf16)
        def run_new():
            fn_v56(
            ...
            fn_s2_bf16(
                mid_o.data_ptr(), output_bf16_new.data_ptr(), sls.data_ptr(),
                mid_o.stride(0), mid_o.stride(1), mid_o.stride(2),
                output_bf16_new.stride(0), output_bf16_new.stride(1),
                NUM_KV_SPLITS, B, Hq,
                ctypes.c_void_p(stream_ptr),
            )

        # Get reference
        run_old(); torch.cuda.synchronize()
        ref = output_bf16_old.float().clone()
        run_new(); torch.cuda.synchronize()
        new = output_bf16_new.float()
        ...
        t_new = profile(run_new)

        tag = "NaN!" if has_nan else (f"cos={cos:.4f}" if cos < 0.999 else "")
        saved = t_old - t_new
        print(f"  B={B:2d} seq={seq_len:5d} | {t_old:>18.1f} us  | {t_new:>14.1f} us  | {saved:>+6.1f}us | {t_sdpa:>5.1f}us | {t_old/t_sdpa:>7.2f}x  | {t_new/t_sdpa:>7.2f}x  {tag}")
