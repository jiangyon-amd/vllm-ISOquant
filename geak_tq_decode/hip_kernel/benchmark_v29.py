...
def run_hip_kernel(so_path, q_rot, kv_cache, bt, sls, centroids_f32, mid_o,
                   B, Hq_l, NUM_KV_SPLITS, BLOCK_KV=16):
...
def main():
    ...
    # Test v12 baseline
    so_v12 = os.path.join(HIP_DIR, "tq_decode_stage1_v12.so")
    mid_o = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v12 = run_hip_kernel(so_v12, q_rot, kv_cache, bt, sls, centroids_f32, mid_o, B, Hq, NUM_KV_SPLITS, BLOCK_KV=8)
    run_v12()
    torch.cuda.synchronize()
    v12_us = bench("HIP v12 (baseline)", run_v12)

    # Test v29
    so_v29 = os.path.join(HIP_DIR, "tq_decode_stage1_v29.so")
    mid_o_v29 = torch.zeros(B, Hq, NUM_KV_SPLITS, D + 1, dtype=torch.float32, device=DEVICE)
    run_v29 = run_hip_kernel(so_v29, q_rot, kv_cache, bt, sls, centroids_f32, mid_o_v29, B, Hq, NUM_KV_SPLITS, BLOCK_KV=16)
    run_v29()
    torch.cuda.synchronize()
    v29_us = bench("HIP v29 (pipelining)", run_v29)
    
    print(f"\n  Speedup v29 vs v12: {v12_us/v29_us:.2f}x")
