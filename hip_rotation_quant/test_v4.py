#!/usr/bin/env python3
"""Test v4 kernel (direct global store)."""
import torch
import ctypes
import os
import sys
...
def test_correctness_v4(M, K, RS=128, shuffle=False):
    ...
    lib.launch_dense_mfma_rot_quant_v4(
...
print("=== V4 (Direct Global Store) Test ===")
print("\n--- Correctness ---")
for M in [1, 32]:
    for K in [128, 4096]:
        fp4_m, sc_m = test_correctness_v4(M, K, shuffle=False)
        status = "✓" if fp4_m > 99 else "✗"
        print(f"  {status} M={M:4d} K={K:4d}: FP4={fp4_m:5.1f}% scale={sc_m:5.1f}%")

print("\n--- Performance (µs, K=4096) ---")
print(f"  {'M':>4} {'v1':>10} {'v4':>10} {'speedup':>10}")
print(f"  {'-'*40}")
for M in [1, 4, 32]:
    ...
    speedup = t_v1 / t_v4
    print(f"  {M:4d} {t_v1:10.1f}µs {t_v4:10.1f}µs {speedup:10.2f}x")
