#!/usr/bin/env python3
"""Test v5 kernel (A prefetching + direct global store)."""
import torch
import ctypes
import os
import sys
...
print("=== V5 (A Prefetch + Direct Store) Test ===")
print("\n--- Correctness ---")
for M in [1, 32]:
    for K in [128, 4096]:
        fp4_m, sc_m = test_correctness(lib, "launch_dense_mfma_rot_quant_v5", M, K, shuffle=False)
        status = "✓" if fp4_m > 99 else "✗"
        print(f"  {status} M={M:4d} K={K:4d}: FP4={fp4_m:5.1f}% scale={sc_m:5.1f}%")

print("\n--- Performance (µs, K=4096) ---")
print(f"  {'M':>4} {'v1':>10} {'v4':>10} {'v5':>10} {'v5/v1':>10}")
print(f"  {'-'*50}")
for M in [1, 4, 32]:
    ...
    t_v5 = benchmark(lib, "launch_dense_mfma_rot_quant_v5", M, 4096, shuffle=False)
    speedup = t_v1 / t_v5
    print(f"  {M:4d} {t_v1:10.1f}µs {t_v4:10.1f}µs {t_v5:10.1f}µs {speedup:10.2f}x")
