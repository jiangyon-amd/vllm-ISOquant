#!/usr/bin/env python3
"""
Test dense MFMA rot+quant HIP kernel vs Gluon v2 reference.
Tests correctness (FP4 + scale match) and performance.
"""
import torch
import ctypes
import subprocess
import os
import sys
import time

DIR = "/data/jiangyon/vllm_rotation/hip_rotation_quant"
HIP = os.path.join(DIR, "dense_mfma_rot_quant.hip")
SO = os.path.join(DIR, "dense_mfma_rot_quant.so")

sys.path.insert(0, "/data/jiangyon/vllm_rotation")


def compile():
    if os.path.exists(SO) and os.path.getmtime(SO) > os.path.getmtime(HIP):
        print("  Already compiled")
        return
    print("  Compiling...")
    r = subprocess.run(
        ["hipcc", "-shared", "-fPIC", "-O2", "--offload-arch=gfx950", "-o", SO, HIP],
        capture_output=True, text=True)
    if r.returncode != 0:
        print(f"  ERROR:\n{r.stderr}")
        sys.exit(1)
    print("  OK")


def test_correctness(lib, M, K, RS=128, shuffle=False):
    """Compare HIP kernel vs Gluon v2 reference."""
    from vllm.model_executor.layers.quantization.quark.fused_rotation_quant_gluon import fused_rot_quant_gluon

    torch.manual_seed(42)
    x = torch.randn(M, K, dtype=torch.bfloat16, device="cuda")
    rot = torch.randn(RS, RS, dtype=torch.bfloat16, device="cuda")

    # Reference: Gluon v2
    fp4_ref, sc_ref = fused_rot_quant_gluon(x, rot, rotation_size=RS, shuffle_scales=shuffle)
    torch.cuda.synchronize()

    # HIP kernel
    fp4_hip = torch.zeros(M, K // 2, dtype=torch.uint8, device="cuda")
    n_scales = K // 32
    if shuffle:
        sn_pad = (n_scales + 7) // 8 * 8
        sm_pad = (M + 255) // 256 * 256
        sc_hip = torch.zeros(sm_pad, sn_pad, dtype=torch.uint8, device="cuda")
    else:
        sn_pad = n_scales
        sc_hip = torch.zeros(M, n_scales, dtype=torch.uint8, device="cuda")

    lib.launch_dense_mfma_rot_quant_v1(
        ctypes.c_void_p(fp4_hip.data_ptr()),
        ctypes.c_void_p(sc_hip.data_ptr()),
        ctypes.c_void_p(x.data_ptr()),
        ctypes.c_void_p(rot.data_ptr()),
        ctypes.c_int(M), ctypes.c_int(K),
        ctypes.c_int(x.stride(0)),
        ctypes.c_int(fp4_hip.stride(0)),
        ctypes.c_int(sc_hip.stride(0)),
        ctypes.c_int(sn_pad),
        ctypes.c_int(1 if shuffle else 0),
        ctypes.c_void_p(0))
    torch.cuda.synchronize()

    # Compare FP4
    fp4_match = (fp4_hip == fp4_ref).float().mean().item() * 100

    # Compare scales
    if shuffle:
        sc_match = (sc_hip[:sc_ref.shape[0], :sc_ref.shape[1]] == sc_ref).float().mean().item() * 100
    else:
        sc_match = (sc_hip == sc_ref).float().mean().item() * 100

    return fp4_match, sc_match


def benchmark(lib, M, K, RS=128, shuffle=False, warmup=50, iters=200):
    """Benchmark HIP kernel latency."""
    torch.manual_seed(0)
    x = torch.randn(M, K, dtype=torch.bfloat16, device="cuda")
    rot = torch.randn(RS, RS, dtype=torch.bfloat16, device="cuda")

    fp4 = torch.zeros(M, K // 2, dtype=torch.uint8, device="cuda")
    n_scales = K // 32
    sn_pad = (n_scales + 7) // 8 * 8 if shuffle else n_scales
    sm_pad = (M + 255) // 256 * 256 if shuffle else M
    sc = torch.zeros(sm_pad, sn_pad, dtype=torch.uint8, device="cuda")

    def run():
        lib.launch_dense_mfma_rot_quant_v1(
            ctypes.c_void_p(fp4.data_ptr()),
            ctypes.c_void_p(sc.data_ptr()),
            ctypes.c_void_p(x.data_ptr()),
            ctypes.c_void_p(rot.data_ptr()),
            ctypes.c_int(M), ctypes.c_int(K),
            ctypes.c_int(x.stride(0)),
            ctypes.c_int(fp4.stride(0)),
            ctypes.c_int(sc.stride(0)),
            ctypes.c_int(sn_pad),
            ctypes.c_int(1 if shuffle else 0),
            ctypes.c_void_p(0))

    for _ in range(warmup):
        run()
    torch.cuda.synchronize()

    start = time.perf_counter()
    for _ in range(iters):
        run()
    torch.cuda.synchronize()
    elapsed = (time.perf_counter() - start) / iters * 1e6
    return elapsed


def test_correctness_v2(lib, M, K, RS=128, shuffle=False):
    """Compare v2 (ds_read_tr) vs Gluon v2 reference."""
    from vllm.model_executor.layers.quantization.quark.fused_rotation_quant_gluon import fused_rot_quant_gluon

    torch.manual_seed(42)
    x = torch.randn(M, K, dtype=torch.bfloat16, device="cuda")
    rot = torch.randn(RS, RS, dtype=torch.bfloat16, device="cuda")

    fp4_ref, sc_ref = fused_rot_quant_gluon(x, rot, rotation_size=RS, shuffle_scales=shuffle)
    torch.cuda.synchronize()

    fp4_hip = torch.zeros(M, K // 2, dtype=torch.uint8, device="cuda")
    n_scales = K // 32
    sn_pad = (n_scales + 7) // 8 * 8 if shuffle else n_scales
    sm_pad = (M + 255) // 256 * 256 if shuffle else M
    sc_hip = torch.zeros(max(sm_pad, M), max(sn_pad, n_scales), dtype=torch.uint8, device="cuda")

    lib.launch_dense_mfma_rot_quant_v2(
        ctypes.c_void_p(fp4_hip.data_ptr()),
        ctypes.c_void_p(sc_hip.data_ptr()),
        ctypes.c_void_p(x.data_ptr()),
        ctypes.c_void_p(rot.data_ptr()),
        ctypes.c_int(M), ctypes.c_int(K),
        ctypes.c_int(x.stride(0)), ctypes.c_int(fp4_hip.stride(0)),
        ctypes.c_int(sc_hip.stride(0)), ctypes.c_int(sn_pad),
        ctypes.c_int(1 if shuffle else 0),
        ctypes.c_void_p(0))
    torch.cuda.synchronize()

    fp4_match = (fp4_hip == fp4_ref).float().mean().item() * 100
    sc_match = (sc_hip[:sc_ref.shape[0], :sc_ref.shape[1]] == sc_ref).float().mean().item() * 100
    return fp4_match, sc_match


def benchmark_v2(lib, M, K, RS=128, shuffle=False, warmup=50, iters=200):
    torch.manual_seed(0)
    x = torch.randn(M, K, dtype=torch.bfloat16, device="cuda")
    rot = torch.randn(RS, RS, dtype=torch.bfloat16, device="cuda")
    fp4 = torch.zeros(M, K // 2, dtype=torch.uint8, device="cuda")
    n_scales = K // 32
    sn_pad = (n_scales + 7) // 8 * 8 if shuffle else n_scales
    sm_pad = (M + 255) // 256 * 256 if shuffle else M
    sc = torch.zeros(max(sm_pad, M), max(sn_pad, n_scales), dtype=torch.uint8, device="cuda")

    def run():
        lib.launch_dense_mfma_rot_quant_v2(
            ctypes.c_void_p(fp4.data_ptr()), ctypes.c_void_p(sc.data_ptr()),
            ctypes.c_void_p(x.data_ptr()), ctypes.c_void_p(rot.data_ptr()),
            ctypes.c_int(M), ctypes.c_int(K),
            ctypes.c_int(x.stride(0)), ctypes.c_int(fp4.stride(0)),
            ctypes.c_int(sc.stride(0)), ctypes.c_int(sn_pad),
            ctypes.c_int(1 if shuffle else 0), ctypes.c_void_p(0))

    for _ in range(warmup):
        run()
    torch.cuda.synchronize()
    start = time.perf_counter()
    for _ in range(iters):
        run()
    torch.cuda.synchronize()
    return (time.perf_counter() - start) / iters * 1e6


def main():
    print("=" * 70)
    print("Dense MFMA Rot+Quant HIP Kernel Test (v1 scalar + v2 ds_read_tr)")
    print("=" * 70)

    compile()
    lib = ctypes.CDLL(SO)

    # v1 correctness
    print("\n--- v1 (scalar B load) Correctness ---")
    for M in [1, 32]:
        for K in [128, 4096]:
            fp4_m, sc_m = test_correctness(lib, M, K, shuffle=False)
            print(f"  M={M:4d} K={K:4d} raw:  FP4={fp4_m:5.1f}% scale={sc_m:5.1f}%")

    # v2 correctness
    print("\n--- v2 (ds_read_tr B load) Correctness ---")
    for M in [1, 32]:
        for K in [128, 4096]:
            fp4_m, sc_m = test_correctness_v2(lib, M, K, shuffle=False)
            status = "✓" if fp4_m > 85 else "✗"
            print(f"  {status} M={M:4d} K={K:4d} raw:  FP4={fp4_m:5.1f}% scale={sc_m:5.1f}%")

    # Performance comparison
    print("\n--- Performance Comparison (µs, K=4096, raw scales) ---")
    print(f"  {'M':>4} {'v1 scalar':>10} {'v2 ds_read_tr':>14} {'Gluon v2':>10} {'v2/v1':>8} {'v2/Gluon':>10}")
    print(f"  {'-'*60}")

    for M in [1, 4, 32]:
        t_v1 = benchmark(lib, M, 4096, shuffle=False)
        t_v2 = benchmark_v2(lib, M, 4096, shuffle=False)
        t_gluon = 14.5
        print(f"  {M:4d} {t_v1:10.1f}µs {t_v2:14.1f}µs {t_gluon:10.1f}µs {t_v2/t_v1:8.2f}x {t_gluon/t_v2:10.2f}x")

    print("\nDone.")


if __name__ == "__main__":
    main()
