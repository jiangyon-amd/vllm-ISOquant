#!/usr/bin/env python3
"""Test the 'best' launcher that auto-selects optimal kernel."""
import torch
import ctypes
import os
import sys
...
print("=" * 70)
print("Best Launcher Test (auto-selects v8 for raw, v1 for shuffled)")
print("=" * 70)

print("\n--- Correctness (raw scales) ---")
for M in [1, 32]:
    for K in [128, 4096]:
        fp4_m, sc_m = test_correctness("launch_dense_mfma_rot_quant_best", M, K, shuffle=False)
...
print(f"  {'M':>4} {'v1':>10} {'best':>10} {'Gluon v2':>10} {'speedup':>10}")
print(f"  {'-'*50}")

for M in [1, 4, 32]:
    t_v1 = benchmark("launch_dense_mfma_rot_quant_v1", M, 4096)
    t_best = benchmark("launch_dense_mfma_rot_quant_best", M, 4096)
    t_gluon = 14.5
    speedup = t_v1 / t_best
    print(f"  {M:4d} {t_v1:10.1f}µs {t_best:10.1f}µs {t_gluon:10.1f}µs {speedup:10.2f}x")

print("\n--- Target Achievement ---")
targets = {1: 5.0, 4: 6.5, 32: 7.0}
for M in [1, 4, 32]:
    t = benchmark("launch_dense_mfma_rot_quant_best", M, 4096)
    target = targets[M]
