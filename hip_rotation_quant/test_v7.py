#!/usr/bin/env python3
"""Test v7 kernel (single wave for M=1)."""
import torch
import ctypes
import os
import sys
...
print("=== V7 (Single Wave for M=1) Test ===")
print("\n--- Correctness ---")
for M in [1, 16, 32]:
    for K in [128, 4096]:
        fp4_m, sc_m = test_correctness(lib, "launch_dense_mfma_rot_quant_v7", M, K, shuffle=False)
        status = "✓" if fp4_m > 99 else "✗"
        print(f"  {status} M={M:4d} K={K:4d}: FP4={fp4_m:5.1f}% scale={sc_m:5.1f}%")

print("\n--- Performance (µs, K=4096) ---")
print(f"  {'M':>4} {'v1':>10} {'v4':>10} {'v7':>10} {'v7/v1':>10}")
print(f"  {'-'*50}")
for M in [1, 4, 16, 32]:
    ...
    t_v7 = benchmark(lib, "launch_dense_mfma_rot_quant_v7", M, 4096, shuffle=False)
    speedup = t_v1 / t_v7
    print(f"  {M:4d} {t_v1:10.1f}µs {t_v4:10.1f}µs {t_v7:10.1f}µs {speedup:10.2f}x")
