...
def generate_input_initializer(tensor_shape: list[int], tensor_dtype: type, input_name: str) -> TensorProto:
    ...
    """
    tensor: NDArray[np.float32] = np.random.normal(0, 0.3, tensor_shape).astype(tensor_dtype)
...
class ModelTransformerTest(unittest.TestCase):
    def _build_model(self) -> ModelProto:
        ...
        relu_node = helper.make_node("Relu", ["Conv1_O"], ["Relu_O"], name="Relu1")
        add_node = helper.make_node("Add", ["Relu_O", "Conv2_O"], ["output"], name="Add1")
    ...
    class ReplaceWholeModel(Transform):
