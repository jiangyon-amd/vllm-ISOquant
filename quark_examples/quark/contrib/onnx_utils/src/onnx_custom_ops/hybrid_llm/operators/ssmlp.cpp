...
SSMlpKernel::SSMlpKernel(
  ...
  if (usingGpu()) {
    try {
      ...
      size_t outputCount = kernel_info.GetOutputCount();
      ...
      std::cerr << "Exception caught in SSMLP constructor: " << e.what()
                << std::endl;
    }
  }
#endif  // ONNX_UTILS_ENABLE_PROJECT_HYBRID_LLM_GPU
  ...
  if (!disableNpuOps()) {
    printLog("NPU Path: SSMLP custom op");
    npu_instance_ = std::make_unique<AMDSSMLPKernel>(info, session_configs);
  }
#endif  // ONNX_UTILS_ENABLE_PROJECT_HYBRID_LLM_NPU

  printLog("Constructed ", nodeName(), " of type SSMlp Custom op\n ");
}
...
SSMlpKernel::~SSMlpKernel() {
  ...
  for (const auto& [event_id, duration] :
       dml_instance->GetPerfData(node_name_)) {
    os << node_name_ << ",GPUSSMLP," << event_id << ","
...
void SSMlpKernel::Compute(OrtKernelContext* context) {
#ifdef ONNX_UTILS_ENABLE_PROJECT_HYBRID_LLM_GPU
  ...
  switch (backend) {
    ...
    case Backend::Gpu: {
      ...
      // PROFILE_THIS(dml_instance->ComputeSSMLPGPU(node_name_, tensor_inputs,
      //                                             tensor_outputs));
