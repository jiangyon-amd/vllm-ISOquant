...
def postprocess_three(
    ...
    size_threshold: int,
    eager_switch_threshold: int = 2048,
) -> None:
    """
    ...
    constant_node_4096 = onnx.helper.make_node(
        "Constant", inputs=[], outputs=["comparison_4096"], value_int=eager_switch_threshold
    )
    new_tvis.append(onnx.helper.make_tensor_value_info("comparison_4096", onnx.TensorProto.INT64, [1]))

    new_nodes.append(constant_node)
    new_nodes.append(constant_node_4096)

    equal_node = onnx.helper.make_node("Equal", inputs=["shape", "comparison"], outputs=["cond_1"])
    new_nodes.append(equal_node)

    equal_node_2 = onnx.helper.make_node("Greater", inputs=["shape", "comparison_4096"], outputs=["cond_4096"])

    new_nodes.append(equal_node_2)
    logical_or = onnx.helper.make_node("Or", inputs=["cond_1", "cond_4096"], outputs=["cond"])
    new_nodes.append(logical_or)

    token_model = ryzenai_onnx_utils.matcher.load_model(token_model_path, load_external_data=False)
    prefill_model = ryzenai_onnx_utils.matcher.load_model(prefill_model_path, load_external_data=False)
    reference_model = ryzenai_onnx_utils.matcher.load_model(reference_model_path, load_external_data=False)
...
def postprocess(
    ...
    if len(input_model_path) == 2:
        ...
        postprocess_three(
            ...
            options.get("eager_switch_threshold", 2048),
        )
