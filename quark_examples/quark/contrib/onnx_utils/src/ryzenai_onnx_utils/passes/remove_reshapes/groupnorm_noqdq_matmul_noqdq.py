...
def replacement(
    ...
    # check if the reshape between groupnorm and matmul is not doing any work
    # i.e. the inner dimensions of groupnorm are combined to form the inner
    # dimension of the matmul
    ...
    # groupnorm to be the same shape as the input to matmul, connect the two
    # and remove the reshape + casts
    matmul_input_tvi: onnx.ValueInfoProto = extractor.vimap[matmul.input[0]]
    group_norm_output_tvi: onnx.ValueInfoProto = extractor.vimap[group_norm.output[0]]
    group_norm_output_tvi.type.tensor_type.shape.CopyFrom(matmul_input_tvi.type.tensor_type.shape)
    for tvi in extractor.model.graph.value_info:
        if tvi.name == group_norm.output[0]:
            tvi.type.tensor_type.shape.CopyFrom(matmul_input_tvi.type.tensor_type.shape)
            break
    matmul.input[0] = group_norm.output[0]

    return [group_norm, matmul], [], None
