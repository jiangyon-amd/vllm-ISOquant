...
def replacement(
    ...
    trs1_node = subgraph[0]
    original_node = subgraph[1]
    trs2_node = subgraph[2]

    assert len(original_node.input) == 3
    assert len(original_node.output) == 1

    (input_shape,) = ryzenai_onnx_utils.matcher.get_shapes(trs1_node.input, extractor)
    ...
    pre_cast_output = trs1_node.input[0] + f".out{pass_id}"
    pre_cast, pre_cast_tvi = add_cast_to_bf16(trs1_node.input[0], pre_cast_output, input_shape, domain)
...
PATTERN = ["Transpose(?,a0)", "Resize([a0,?],b0)", "Transpose(b0,?)"]
