...
def replacement(
    ...
    mha_q_shape = ryzenai_onnx_utils.matcher.get_shape(mha.input[0], extractor)
    mha_k_shape = ryzenai_onnx_utils.matcher.get_shape(mha.input[1], extractor)
    mha_v_shape = ryzenai_onnx_utils.matcher.get_shape(mha.input[2], extractor)
    qo_bias_tvi = onnx.helper.make_tensor_value_info(mha_bias + "_q_bias", dtype, mha_q_shape)
    ko_bias_tvi = onnx.helper.make_tensor_value_info(mha_bias + "_k_bias", dtype, mha_k_shape)
    vo_bias_tvi = onnx.helper.make_tensor_value_info(mha_bias + "_v_bias", dtype, mha_v_shape)
    ...
    return (
        ...
        [q_bias_tvi, k_bias_tvi, v_bias_tvi, qo_bias_tvi, ko_bias_tvi, vo_bias_tvi],
    )
